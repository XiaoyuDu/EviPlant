  ...
  for entryObject in directoryObject:
      ...
      try:
        ...
        if f_type == pytsk3.TSK_FS_META_TYPE_DIR:
            ...
            #get all the file and its md5hash and put them into hashmap
...
# reverse the hashmap, the items with more than one value are the target image(with same md5hash)
rev_hashMap = {}
for key, value in hashMap.items():
  rev_hashMap.setdefault(value, list()).append(key)

for v in [values for key, values in rev_hashMap.items() if len(values) > 1]:
  extractFile(v)
