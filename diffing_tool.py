...
def directoryRecurse(filesystemObject,directoryObject, parentPath, sha1Hash):
  ...
  for entryObject in directoryObject:
      ...
      try:
        ...
        # filepath = os.sep + os.sep.join(parentPath) + os.sep + entryObject.info.name.name
        ...
        if f_type == pytsk3.TSK_FS_META_TYPE_DIR:
          ...
          # in the dic, the key is sha1hash, value is the filePath(used to find out the file according to sha1hash)
            ...
            wr.writerow([int(entryObject.info.meta.addr),'/'.join(parentPath) + os.sep + entryObject.info.name.name,datetime.datetime.fromtimestamp(entryObject.info.meta.crtime).strftime('%Y-%m-%d %H:%M:%S'),int(entryObject.info.meta.size),md5hash.hexdigest(),sha1hash.hexdigest()])

            # wr.writerow([int(entryObject.info.meta.addr),os.sep.join(parentPath) + os.sep + entryObject.info.name.name,datetime.datetime.fromtimestamp(entryObject.info.meta.crtime).strftime('%Y-%m-%d %H:%M:%S'),int(entryObject.info.meta.size),md5hash.hexdigest(),sha1hash.hexdigest()])

        elif f_type == pytsk3.TSK_FS_META_TYPE_REG and entryObject.info.meta.size == 0:

            wr.writerow([int(entryObject.info.meta.addr),'/'.join(parentPath)+ os.sep +entryObject.info.name.name,datetime.datetime.fromtimestamp(entryObject.info.meta.crtime).strftime('%Y-%m-%d %H:%M:%S'),int(entryObject.info.meta.size),"d41d8cd98f00b204e9800998ecf8427e","da39a3ee5e6b4b0d3255bfef95601890afd80709"])
            # wr.writerow([int(entryObject.info.meta.addr),os.sep.join(parentPath)+ os.sep +entryObject.info.name.name,datetime.datetime.fromtimestamp(entryObject.info.meta.crtime).strftime('%Y-%m-%d %H:%M:%S'),int(entryObject.info.meta.size),"d41d8cd98f00b204e9800998ecf8427e","da39a3ee5e6b4b0d3255bfef95601890afd80709"])
